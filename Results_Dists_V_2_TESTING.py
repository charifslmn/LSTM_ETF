...
def add_threshold_metrics(data: List[Dict] ,threshold_lower: float , threshold_upper: float) -> None:
    ...
    for res in data:
        ...
        pred_threshold_sigmoid01_up_upper = threshold_upper
        pred_threshold_sigmoid01_up_lower = threshold_lower
        ...
        for p_fold, a_fold, HOD_a_fold, UCO_a_fold, HUC_a_fold, CRUD_a_fold, USO_a_fold in zip(all_preds, actuals_per_fold, HOD_per_fold, UCO_per_fold, HUC_per_fold, CRUD_per_fold, USO_per_fold):
            ...
            for p, a, HOD_a, UCO_a, HUC_a, CRUD_a, USO_a in zip(p_fold, a_fold, HOD_a_fold, UCO_a_fold, HUC_a_fold, CRUD_a_fold, USO_a_fold):
                if p > 0.5 and (pred_threshold_sigmoid01_up_lower < p < pred_threshold_sigmoid01_up_upper):
                ...
                elif p > 0.5 and ((p <= pred_threshold_sigmoid01_up_lower)  or (p >= pred_threshold_sigmoid01_up_upper)):
        ...
        for row_raw_actuals, row_01_actuals, row_preds, row_preds_thresh , row_HUC_vals , row_HOD_vals , row_UCO_vals , row_CRUD_vals , row_USO_vals in zip(
            ...
            for entry_raw_actual, entry_01_actual, entry_pred , entry_HOD_actual, entry_UCO_actual, entry_HUC_actual, entry_CRUD_actual, entry_USO_actual in zip(row_raw_actuals_flattened, row_01_actuals_flattened, row_preds_flattened \
                ...
                if entry_pred > 0.5 and entry_01_actual < 0.5:
                    false_up_preds_row_actual.append(round(entry_raw_actual, 3)) 
                    false_up_preds_row_probabs.append(round(entry_pred,3))
                    HOD_false_up_preds_col_row_actual.append(round(entry_HOD_actual, 3))
                    UCO_false_up_preds_col_row_actual.append(round(entry_UCO_actual, 3))
                    HUC_false_up_preds_col_row_actual.append(round(entry_HUC_actual, 3))
            ...
            for entry_raw_actual, entry_01_actual, entry_pred , entry_HOD_actual, entry_UCO_actual, entry_HUC_actual in zip(row_raw_actuals_flattened, row_01_actuals_flattened, row_preds_flattened \
                ...
                if entry_pred > 0.5 and entry_01_actual > 0.5:
                    correct_up_preds_row_actual.append(round(entry_raw_actual, 3)) 
                    correct_up_preds_row_probabs.append(round(entry_pred,3))
                    HOD_correct_up_preds_col_row_actual.append(round(entry_HOD_actual, 3))
                    UCO_correct_up_preds_col_row_actual.append(round(entry_UCO_actual, 3))
                    HUC_correct_up_preds_col_row_actual.append(round(entry_HUC_actual, 3))
                    CRUD_correct_up_preds_col_row_actual.append(round(entry_CRUD_actual, 3))
                    USO_correct_up_preds_col_row_actual.append(round(entry_USO_actual, 3))
            ...
            for entry_raw_actual, entry_01_actual, entry_pred , entry_HOD_actual, entry_UCO_actual, entry_HUC_actual, entry_CRUD_actual, entry_USO_actual in zip(row_raw_actuals_flattened, row_01_actuals_flattened, row_preds_thresh_flattened \
                ...
                if isinstance(entry_pred, (int, float)) and entry_pred > 0.5 and entry_01_actual < 0.5:
                    false_up_preds_row_actual_thresh.append(round(entry_raw_actual, 3)) 
                    false_up_preds_row_probabs_thresh.append(round(entry_pred,3))
                    HOD_false_up_preds_col_row_actual_thresh.append(round(entry_HOD_actual, 3))
                    UCO_false_up_preds_col_row_actual_thresh.append(round(entry_UCO_actual, 3))
                    HUC_false_up_preds_col_row_actual_thresh.append(round(entry_HUC_actual, 3))
                    CRUD_false_up_preds_col_row_actual_thresh.append(round(entry_CRUD_actual, 3))
                    USO_false_up_preds_col_row_actual_thresh.append(round(entry_USO_actual, 3))
            ...
            for entry_raw_actual, entry_01_actual, entry_pred , entry_HOD_actual, entry_UCO_actual, entry_HUC_actual, entry_CRUD_actual, entry_USO_actual in zip(row_raw_actuals_flattened, row_01_actuals_flattened, row_preds_thresh_flattened \
                ...
                if isinstance(entry_pred, (int, float)) and entry_pred > 0.5 and entry_01_actual > 0.5:
                    correct_up_preds_row_actual_thresh.append(round(entry_raw_actual, 3)) 
                    correct_up_preds_row_probabs_thresh.append(round(entry_pred,3))
                    HOD_correct_up_preds_col_row_actual_thresh.append(round(entry_HOD_actual, 3))
                    UCO_correct_up_preds_col_row_actual_thresh.append(round(entry_UCO_actual, 3))
                    HUC_correct_up_preds_col_row_actual_thresh.append(round(entry_HUC_actual, 3))
                    CRUD_correct_up_preds_col_row_actual_thresh.append(round(entry_CRUD_actual, 3))
                    USO_correct_up_preds_col_row_actual_thresh.append(round(entry_USO_actual, 3))
...
def process_func_PLUS_return_analytics(master_df: pd.DataFrame, 
    ...
    for machine, data_type, corr_range, group_size in groups_config:
        # Create group name based on parameters
        group_name = f"pair_{group_size}_{corr_range[0]}_{corr_range[1]}".replace('.', '').replace('-', 'neg')
        
        # Get model groups
        groups = get_model_groups_in_corr_range(master_df, machine, data_type, corr_range, group_size=group_size)
...
def process_func_PLUS_return_analytics_THRESH_var_included( #master_df: pd.DataFrame, 
    ...
    for data in names_all.values():
        add_threshold_metrics(data, threshold_lower=threshold_lower, threshold_upper=threshold_upper)
