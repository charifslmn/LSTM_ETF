...
def run_combo(INDEX, combo, total_offset , use_print_acc_vs_pred : bool):
    ...
    def GRIDSEARCH_FUNCTION_WITH_CV_forTS(
        ...
        use_binary_0_1_custom_neg : bool ,
        use_binary_0_1_custom_pos : bool ,
        binary_0_1_cutoff_ret_rate_percentage : float,  ### cutoff for the  use_binary_0_1_custom_pos ot use_binary_0_1_custom_neg
...
######### --------------------------------------- DATA IMPORT ---------------------------------------
       ...
       #   IMPORTANT IMPORTANT                       --- Use copies of each DataFrame to avoid in-place modification of pickled data---                    ********* IMPORTANT IMPORTANT IMPORTANT 
        ...
        if use_binary_0_1 : predictor_data_wti_vals = [1 if val > 0 else 0 for val in predictor_data_wti_vals]
...
######### --------------------------------------- MODEL TRAINING AND EVALUATION  ---------------------------------------
        ...
        for set_idx, (train_loader, X_val, Y_val) in enumerate(zip(train_loader_LIST, X_vals_LIST, Y_vals_LIST)):
            ...
            if use_class_weighting and use_binary_0_1 and use_dynamic_weights: 
            ...
            elif use_class_weighting and use_binary_0_1_custom_pos:
            ...
            elif use_class_weighting and use_binary_0_1_custom_neg:
...
##############################################################################################################################
        ...
        # for i in range(iter_per_valSET):
            ...
            if use_binary_0_1 or use_binary_0_1_custom_pos or use_binary_0_1_custom_neg:
            ...
            with torch.no_grad():
                ...
                val_predictions = torch.sigmoid(val_output).detach().cpu().numpy().flatten() if (use_binary_0_1 or use_binary_0_1_custom_neg or use_binary_0_1_custom_pos) \
            ...
            if use_binary_0_1 or use_binary_0_1_custom_neg or use_binary_0_1_custom_pos:
...
def run_combo_V_2(INDEX, combo, total_offset , use_print_acc_vs_pred : bool , pred_threshold_sigmoid01_up_bool : bool):
...
######### --------------------------------------- DATA IMPORT ---------------------------------------
       ...
       #   IMPORTANT IMPORTANT                       --- Use copies of each DataFrame to avoid in-place modification of pickled data---                    ********* IMPORTANT IMPORTANT IMPORTANT 
        ...
        if use_binary_0_1_custom_neg : 
            ...
            null = torch.rand((100, len(predictor_data_wti_vals))).gt(0.9)
            targets = torch.tensor(predictor_data_wti_vals).repeat(100, 1).to(torch.bool)

            true_positives = torch.bitwise_and(null, targets).float().sum(dim=1)
            total_positives = targets.float().sum(dim=1)      
            # total_up_predictions = null.float().sum(dim=1)

            # predictions = (null == targets).float().mean(dim=1)
            # print(predictions)
...
##############################################################################################################################
        ...
        if  (pred_threshold_sigmoid01_up_bool) and (use_binary_0_1_custom_neg or use_binary_0_1_custom_pos):
...
def run_combo_V_3(INDEX, combo, total_offset , use_print_acc_vs_pred : bool , pred_threshold_sigmoid01_up_bool : bool):
...
######### --------------------------------------- DATA IMPORT ---------------------------------------
       ...
       #   IMPORTANT IMPORTANT                       --- Use copies of each DataFrame to avoid in-place modification of pickled data---                    ********* IMPORTANT IMPORTANT IMPORTANT 
        ...
        if use_binary_neg1_1 : 
            predictor_data_wti_vals = [1 if val > 0 else -1 for val in predictor_data_wti_vals]
        if use_ret_rate : 
            predictor_data_wti_vals = predictor_data_wti_vals
...
######### --------------------------------------- DATA LOADERS  - TRAIN/TEST/VAL ---------------------------------------
        ...
        train_loader_LIST_RAW_Y_VALS = []  ### NEW NEW NEW
...
# ---------------- SAVE DATA FOR ANALYSIS 
        ...
        for i in range(num_folds):
            ...
            Y_TRAIN_RAW = torch.tensor(raw_actuals_full[      i   *   units_per_val_set      :cut_idxs[i]]  ).float()  ### NEW NEW NEW
